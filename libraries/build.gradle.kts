...
if (androidxDirectory != null && gradle.parent == null) {
    ...
    val checkoutAndroidx by tasks.registering {
        ...
        exec {
            workingDir = file(androidxDirectory)
            commandLine =
                listOf("git", "checkout", localProperties["project.androidx.commitId"] as String)
        }
    }
    tasks.register("syncUpstream") {
        ...
        doLast {
            val upstream =
                rootProject.file("$androidxDirectory/compose/material/material/src/commonMain/kotlin/androidx/compose/material/pullrefresh")
            val local =
                rootProject.file("library/src/main/kotlin/androidx/compose/material3/pullrefresh")
